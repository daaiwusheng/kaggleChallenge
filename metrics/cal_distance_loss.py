...
class DistanceLossCalculator(nn.Module):
    def __init__(self, dict_weights=dict_loss_weights, split_channels:list=[1,1,1]):
        super(DistanceLossCalculator, self).__init__()
    ...
    def forward(self, predictions, distance_y):
        predictions_splited = self.splitor(predictions)

        loss_result = self.dict_weights[WEIGHT_DISTANCE_KEY] * self.distance_loss(torch.tanh(predictions_splited[2]), distance_y)
        return loss_result
